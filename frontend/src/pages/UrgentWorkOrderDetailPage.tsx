...
const UrgentWorkOrderDetailPage: React.FC = () => {
  ...
  const { data: workOrder, isLoading, error } = useQuery({
    queryKey: ['urgentWorkOrder', id],
    queryFn: async () => {
      const res = await api.get<UrgentWorkOrderResponse>(`/api/urgent-work-orders/${id}`);
      return res.data;
    },
    enabled: !!id,
  });

  // Fetch mileage entries linked to this urgent work order
  const { data: mileageEntries, isLoading: mileageLoading } = useQuery({
    queryKey: ['mileageEntriesUrgent', id],
    queryFn: async () => {
      const res = await api.get('/api/mileage', { params: { urgentWorkOrderId: id } });
      return res.data;
    },
    enabled: !!id,
  });
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'OPEN': return isDark ? 'bg-teal-500/15 text-teal-300 ring-1 ring-teal-500/25' : 'bg-teal-50 text-teal-700 ring-1 ring-teal-200';
      case 'ASSIGNED': return isDark ? 'bg-blue-500/15 text-blue-300 ring-1 ring-blue-500/25' : 'bg-blue-50 text-blue-700 ring-1 ring-blue-200';
      case 'IN_PROGRESS': return isDark ? 'bg-amber-500/15 text-amber-300 ring-1 ring-amber-500/25' : 'bg-amber-50 text-amber-700 ring-1 ring-amber-200';
      case 'COMPLETED': return isDark ? 'bg-emerald-500/15 text-emerald-300 ring-1 ring-emerald-500/25' : 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200';
      case 'CANCELLED': return isDark ? 'bg-red-500/15 text-red-300 ring-1 ring-red-500/25' : 'bg-red-50 text-red-700 ring-1 ring-red-200';
    ...
    <div className={`p-4 sm:p-6 lg:p-8 pb-10 sm:pb-12 printable-content ${isDark ? 'bg-surface-950 text-surface-100' : 'bg-surface-50 text-gray-900'}`}>
      ...
      <style>{`
        @media print {
          ...
          .printable-content { position: absolute; left: 0; top: 0; width: 100vw; background: white; color: black; }
      ...
      <div className="max-w-5xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2 text-sm font-mono opacity-60">
                  <span>{t.urgentWoNumber} {workOrder.id}</span>
                  <span>•</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-0">
                  ...
                  <div className={`rounded-xl overflow-hidden border ${isDark ? 'bg-surface-950 border-surface-700' : 'bg-green-50 border-green-200'}`}>
                    {isInvoiceImage ? (
                      <div className="relative group">
                        ...
                        <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex justify-end">
                          <button type="button" onClick={() => openSecureFile(invoiceUrl)} className="bg-white text-gray-900 px-4 py-2 rounded-lg text-sm font-semibold shadow hover:bg-gray-100">
                            {t.openOriginal}
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="p-6 flex items-center justify-between">
                        ...
                        <button type="button" onClick={() => downloadSecureFile(invoiceUrl, workOrder.invoiceFilename || 'invoice')} className={`px-4 py-2 rounded-lg text-sm font-semibold border transition-colors ${isDark ? 'border-gray-600 hover:bg-gray-700' : 'border-green-300 bg-green-100 hover:bg-green-200 text-green-800'}`}>
                          {t.download}
                        </button>
                      </div>
                    )}
            ...
            <div className={`p-6 sm:p-8 pb-10 sm:pb-12 space-y-4 ${isDark ? 'bg-surface-800/50' : 'bg-gray-50/30'}`}>
              ...
              <div>
                ...
                <p className="text-sm opacity-80">{workOrder.createdAt ? new Date(workOrder.createdAt).toLocaleString() : '—'}</p>
              </div>
              <div>
        ...
        {/* Action buttons below main content, matching WorkOrderDetailPage */}
        <div className="max-w-5xl mx-auto mt-8 flex justify-center gap-4 no-print" data-html2canvas-ignore="true">
          ...
          <button
            ...
            onClick={async () => {
              ...
              // Use explicit style object to backup instead of getAttribute for cleaner restoration
              ...
              try {
                ...
                if (imgHeight > pageHeight) {
                   ...
                   pdfMulti.save(`UrgentWorkOrder_${workOrder?.id ?? 'Detail'}.pdf`);
                } else {
                   // Single page
                   pdf.addImage(imgData, 'PNG', 0, 0, imgWidth, imgHeight);
                   pdf.save(`UrgentWorkOrder_${workOrder?.id ?? 'Detail'}.pdf`);
                }
              } finally {
...
export default UrgentWorkOrderDetailPage;
