        ...
        <div className="w-full">
          <div className="w-full flex items-start gap-3 relative">
            ...
            <div className="flex-shrink-0 flex flex-col gap-0 self-stretch">
              ...
              <button
                onClick={async () => {
                  const res = await api.get('/api/admin/work-orders/export', { responseType: 'blob' });
                  const url = URL.createObjectURL(new Blob([res.data], { type: 'text/csv' }));
                  const a = document.createElement('a');
                  a.href = url; a.download = 'bons-de-travail.csv'; a.click();
                  URL.revokeObjectURL(url);
                }}
                className={
                  colorScheme === 'dark'
                    ? 'bg-surface-700 text-surface-100 border border-surface-600 px-3 sm:px-4 py-1 rounded-lg shadow hover:bg-surface-600 transition-all duration-200 font-semibold text-xs sm:text-sm flex items-center justify-center gap-1 whitespace-nowrap mt-1'
                    : 'bg-gray-100 text-gray-700 border border-gray-300 px-3 sm:px-4 py-1 rounded-lg shadow hover:bg-gray-200 transition-all duration-200 font-semibold text-xs sm:text-sm flex items-center justify-center gap-1 whitespace-nowrap mt-1'
                }
                title={t.exportCsv}
              >
                <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
